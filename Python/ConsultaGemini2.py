...
def consultar_gemini(prompt_ia, datos):
    print(f"📝 Generando contenido con el prompt...")
    time.sleep(0.6)
...
def procesar_json(ruta_archivo):
    print(f"📁 Cargando archivo JSON desde: {ruta_archivo}")
    time.sleep(0.6)
    
    if not os.path.exists(ruta_archivo) or os.path.getsize(ruta_archivo) == 0:
        print(f"El archivo no existe o está vacío")
        return
    ...
    if respuesta:
        ...
        print("📨 Respuesta de Gemini obtenida")
        time.sleep(0.6)
...
ruta = "C:/Proyectos/Demo-Huawei-DeepSeek/Json/consultaBeneficiosGIA.json"
procesar_json(ruta)
